...
#: Name and filename of the generated country lookup, when a corpus needs one.
_GEOIP_KEYS: tuple[str, str] = ("sagan_geoip_keys", "sagan-geoip-keys.vrl")

#: The generated transform's filename, public because the engine lab builds a
#: runnable pipeline from `pipeline_transforms` and has to know which entry it
#: must generate rather than read from the package.
GENERATED_GEOIP_KEYS = _GEOIP_KEYS[1]

_GEOIP_KEYS_HEADER = """\
# Country of the addresses a rule binds by name, generated by sagan2sigma.
#
# Why this file is generated rather than shipped
# ----------------------------------------------
# `sagan-geoip.vrl` resolves the country of the addresses Parse_IP finds by
# scanning the message. A rule carrying `json_map: "src_ip", ".ClientIP"` does
# not use those: the engine reads the key and looks up that value alone, so a
# converted rule testing the parsed address would answer about a different
# machine whenever a document carries more than one address. Measured on an
# Entra sign-in document holding an earlier address, the parsed one was 5.5.5.5
# and the bound one 203.0.113.9, two different countries.
#
# So the keys below are the ones the converted rules actually read, taken from
# the corpus that produced them. Regenerating the rules regenerates this file;
# a key no rule binds is not looked up.
#
# Each block sets <key>_country, the field name `mapping/geoip.py` emits.

"""

_GEOIP_KEYS_BLOCK = """\
address = string(%(path)s) ?? ""
if address != "" {
    record, err = get_enrichment_table_record("sagan_geoip", {"ip": address})
    if err == null {
        code = string(record.country_code) ?? string(record.country.iso_code) ?? ""
        if code != "" { %(field)s = code }
    }
}
...
def _vrl_path(key: str) -> str:
    """A JSON key as a VRL path, quoting each segment that needs it."""
    parts = key.split(".")
    return "." + ".".join(
        part if part.isidentifier() else f'"{part}"' for part in parts
    )


def country_lookups(keys: Iterable[str]) -> str:
    """The generated transform resolving the country of each bound key.

    Built from `mapping.geoip.country_field_for` rather than from a second
    spelling of the same convention, so the field a rule tests and the field the
    pipeline sets cannot drift apart.
    """
    from ..mapping.geoip import country_field_for

    body = "".join(
        _GEOIP_KEYS_BLOCK
        % {"path": _vrl_path(key), "field": _vrl_path(country_field_for(key))}
        for key in sorted(keys)
    )
    return _GEOIP_KEYS_HEADER + body
...
def pipeline_transforms(
    flags: dict[str, bool], geoip_keys: bool = False
) -> list[tuple[str, str]]:
    ...
    return [
        ...
        *([_GEOIP_KEYS] if geoip_keys else []),
        _USERNAME,
    ]
...
def build_config(
    ...
    blocks: list[str] = []
    previous = source
    for name, filename in pipeline_transforms(flags, geoip_keys=geoip_keys):
        blocks.append(
...
def write_pipeline(
    ...
    config_path.write_text(
        build_config(
            ...
            zeek=zeek,
            bluedot=bluedot,
            geoip_keys=bool(keys),
        ),
        encoding="utf-8",
    )

    written = [config_path]
    for _, filename in pipeline_transforms(flags, geoip_keys=bool(keys)):
        path = transforms_dir / filename
        text = (
            country_lookups(keys)
            if filename == _GEOIP_KEYS[1]
