...
| --- | --- | --- | --- |
| sagan-rules | `78148f1198717ddf1a0cdaeec91822e30b39324e` | 2026-09-14T12:46:38-04:00 | https://github.com/quadrantsec/sagan-rules.git |
| SigmaHQ | `5c9b21756f4e3ba137c1773ac9ba5a8332188961` | 2026-09-07T12:54:20+02:00 | https://github.com/SigmaHQ/sigma.git |

Total overlapping pairs listed: **2464**.
...
| --- | ---: | --- | --- |
| **Confirmed by both analyses** | 13 | overlap engine + witness invariant, and conceptual | The engine confirmed both rules fire on one synthesised event, they are log-source compatible, and independently the conceptual analysis found them searching for the same distinctive terms. Strongest evidence available; review these first. |
| **Behaviourally confirmed coverage (tested)** | 53 | overlap engine + witness-fires-both invariant | Every event synthesised from the converted rule also fired the SigmaHQ rule (or each fired all of the other's), log sources are compatible, and the witness event is attached and replayable. Deploying SigmaHQ makes the converted rule redundant on the evidence. |
| **Behaviourally related, not coverage (tested)** | 19 | overlap engine (co-firing tested) | The two fired together on at least one event, log sources are compatible, but neither contains the other (an overlap), or the converted rule is the broader of the two. Related, not interchangeable. |
| **Cross-log-source co-firing (tested, not deployable)** | 555 | overlap engine (co-firing tested); log-source gate | The engine confirmed the two fire on one event, but their log sources differ, so in production the SigmaHQ rule would not see that event. Usually a SigmaHQ keyword rule matching a common word in another product's raw text. Not deployable coverage; recorded for completeness. |
| **Conceptual candidate, strong lexical match (review)** | 383 | conceptual lexical similarity only | No behavioural co-firing was found, but the two rules share distinctive search terms strongly enough to suggest they detect the same thing. A lead for human review, not a tested fact. |
| **Conceptual candidate, weaker lexical match (review)** | 1441 | conceptual lexical similarity only | A weaker lexical similarity, near the floor. A candidate to skim, most useful read alongside its shared terms. |

## Confirmed by both analyses (13)
...
| `5003764` | [WINDOWS-SECURITY] An attempt was made to set the Direc | Password Change on Directory Service Restore Mode (DSRM | `rules/windows/builtin/security/win_security_susp_dsrm_password_change.yml` | `SAGAN_REDUNDANT` | 2 | restore, made, mode, administrator, password, set |
| `5003767` | [WINDOWS-SECURITY] A replay attack was detected | Replay Attack Detected | `rules/windows/builtin/security/win_security_replay_attack_detected.yml` | `SAGAN_REDUNDANT` | 2 | replay, attack |
| `5002090` | [WINDOWS-APPLOCKER] Allowed program to execute | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | `SAGAN_REDUNDANT` | 2 | applocker |
| `5002093` | [WINDOWS-APPLOCKER] Allowed MSI/Script, but would have | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | `SAGAN_REDUNDANT` | 2 | applocker, blocked |
| `5002091` | [WINDOWS-APPLOCKER] Application blocked | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | `SAGAN_REDUNDANT` | 2 | applocker |
| `5002096` | [WINDOWS-APPLOCKER] Package application audited | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | `SAGAN_REDUNDANT` | 2 | applocker |
| `5002099` | [WINDOWS-APPLOCKER] Package application installation au | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | `SAGAN_REDUNDANT` | 2 | applocker |
| `5002097` | [WINDOWS-APPLOCKER] Package application disabled | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | `SAGAN_REDUNDANT` | 2 | applocker |
...
| `5000123` | [SYSLOG] Oversized packet - ping of death? | Suspicious Log Entries | `rules/linux/builtin/lnx_shell_susp_log_entries.yml` | `SAGAN_REDUNDANT` | 1 | oversized, received, packet |

## Behaviourally confirmed coverage (tested) (53)
...
| `5002094` | [WINDOWS-APPLOCKER] Prevent MSI/Script to execute | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | `SAGAN_REDUNDANT` | 2 |  |
| `5002100` | [WINDOWS-APPLOCKER] Package application installation di | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | `SAGAN_REDUNDANT` | 2 |  |
| `5003769` | [WINDOWS-SECURITY] SID History was added to an account | Addition of SID History to Active Directory Object | `rules/windows/builtin/security/win_security_susp_add_sid_history.yml` | `SAGAN_REDUNDANT` | 2 |  |
| `5003770` | [WINDOWS-SECURITY] An attempt to add SID History to an | Addition of SID History to Active Directory Object | `rules/windows/builtin/security/win_security_susp_add_sid_history.yml` | `SAGAN_REDUNDANT` | 2 |  |
...
| `5017936` | [WINDOWS-SYSMON] Windows Defender has detected malware | Windows Defender Threat Detected | `rules/windows/builtin/windefend/win_defender_threat.yml` | `SAGAN_REDUNDANT` | 1 |  |
| `5017937` | [WINDOWS-SYSMON] Windows Defender has detected potentia | Windows Defender Threat Detected | `rules/windows/builtin/windefend/win_defender_threat.yml` | `SAGAN_REDUNDANT` | 1 |  |
...
## Behaviourally related, not coverage (tested) (19)
...
| Sagan SID | Converted rule | SigmaHQ rule | SigmaHQ path | Relation | Events | Shared terms |
| --- | --- | --- | --- | --- | ---: | --- |
| `5001880` | [WINDOWS-AUTH] User account created | Local User Creation | `rules/windows/builtin/security/win_security_user_creation.yml` | `OVERLAP` | 4 |  |
...
| `5003397` | [WINDOWS-SECURITY] A new trust was created to a domain | A New Trust Was Created To A Domain | `rules/windows/builtin/security/win_security_susp_add_domain_trust.yml` | `OVERLAP` | 4 |  |
...
| `5000111` | [CISCO-IOS] IOS configuration changed | Exploitation Indicators Of CVE-2023-20198 | `rules-emerging-threats/2023/Exploits/CVE-2023-20198/cisco_syslog_cve_2023_20198_ios_xe_web_ui.yml` | `OVERLAP` | 1 |  |
...
## Cross-log-source co-firing (tested, not deployable) (555)
...
| `5017164` | [CROWDSTRIKE] Network Access In An Epp Detection Summar | Relevant Anti-Virus Signature Keywords In Application L | `rules/windows/builtin/application/Other/win_av_relevant_match.yml` | `OVERLAP` | 4 |  |
...
| `5100164` | Azure Eventhub Active Directory detected | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | `OVERLAP` | 4 |  |
...
| `5003203` | [WINDOWS-AUTH] SAM Database Unable to Lock Account | Cisco Discovery | `rules/network/cisco/aaa/cisco_cli_discovery.yml` | `SAGAN_REDUNDANT` | 2 |  |
| `5010325` | [DYNAMIC] MSAPI-AzureAD logs detected via program | Cisco Discovery | `rules/network/cisco/aaa/cisco_cli_discovery.yml` | `SAGAN_REDUNDANT` | 2 |  |
| `5013708` | MSAPI AzureAD device detected | Cisco Discovery | `rules/network/cisco/aaa/cisco_cli_discovery.yml` | `SAGAN_REDUNDANT` | 2 |  |
| `5013730` | [DYNAMIC] RansomCare logs detected via program. | Relevant Anti-Virus Signature Keywords In Application L | `rules/windows/builtin/application/Other/win_av_relevant_match.yml` | `SAGAN_REDUNDANT` | 2 |  |
...
| `5017819` | [GCP] Multiple SecretManager Secrets Deleted | Cisco File Deletion | `rules/network/cisco/aaa/cisco_cli_file_deletion.yml` | `OVERLAP` | 2 |  |
...
| `5014359` | [FORTINET] Botnet Communication Detected - Direction Ou | Cisco Discovery | `rules/network/cisco/aaa/cisco_cli_discovery.yml` | `SAGAN_REDUNDANT` | 1 |  |
...
| `5014601` | [WINDOWS-SYSMON] Use Of ProcDump To Dump LSASS Detected | Suspicious SQL Query | `rules/category/database/db_anomalous_query.yml` | `SAGAN_REDUNDANT` | 1 |  |
...
| `5017301` | [FORTINET] Botnet Communication Detected - Direction Ou | Cisco Discovery | `rules/network/cisco/aaa/cisco_cli_discovery.yml` | `SAGAN_REDUNDANT` | 1 |  |
...
## Conceptual candidate, strong lexical match (review) (383)
...
| `5010732` | [WINDOWS-POWERSHELL] Possible obfuscated script with mu | Split A File Into Pieces | `rules/macos/process_creation/proc_creation_macos_split_file_into_pieces.yml` | 0.84 | split |
| `5010733` | [WINDOWS-POWERSHELL] Possible obfuscated script with mu | Split A File Into Pieces | `rules/macos/process_creation/proc_creation_macos_split_file_into_pieces.yml` | 0.84 | split |
...
| `5017968` | [WINDOWS-SECURITY] Kerberos - SPN Enumeration via setsp | Potential SPN Enumeration Via Setspn.EXE | `rules/windows/process_creation/proc_creation_win_setspn_spn_enumeration.yml` | 0.81 | setspn.exe, spn, enumeration |
...
| `5100011` | bash shell in use | Linux Shell Pipe to Shell | `rules/linux/process_creation/proc_creation_lnx_susp_pipe_shell.yml` | 0.76 | bash, shell |
...
| `5100106` | Microsoft MSSQL server detected | MSSQL Server Failed Logon | `rules/windows/builtin/application/mssqlserver/win_mssql_failed_logon.yml` | 0.75 | mssql, server |
...
| `5100011` | bash shell in use | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.73 | bash, shell |
...
| `5003403` | [WINDOWS-SECURITY] A security-enabled global group was | A Security-Enabled Global Group Was Deleted | `rules/windows/builtin/security/account_management/win_security_security_enabled_global_group_deleted.yml` | 0.73 | security-enabled, global, group |
...
| `5003015` | [DYNAMIC] MSSQL logs detected via program. | MSSQL Server Failed Logon | `rules/windows/builtin/application/mssqlserver/win_mssql_failed_logon.yml` | 0.70 | mssql |
...
| `5008821` | [WINDOWS-MALWARE] Various ransomware file extension det | OneLogin User Account Locked | `rules/identity/onelogin/onelogin_user_account_locked.yml` | 0.69 | locked |
| `5009015` | [WINDOWS-MALWARE] Various ransomware file extension det | OneLogin User Account Locked | `rules/identity/onelogin/onelogin_user_account_locked.yml` | 0.69 | locked |
...
| `5017965` | [WINDOWS-SECURITY] Kerberos - AS-REP Roasting Attempt w | Potential AS-REP Roasting via Kerberos TGT Requests | `rules/windows/builtin/security/win_security_kerberos_asrep_roasting.yml` | 0.67 | as-rep, pre-authentication, roasting, 0x17, ticket, encryption |
...
| `5015506` | [WINDOWS-SECURITY] CRITICAL - Microsoft Quick Assist Ex | QuickAssist Execution | `rules/windows/process_creation/proc_creation_win_quickassist_execution.yml` | 0.67 | assist, quickassist.exe, quick, microsoft |
...
| `5002963` | [DYNAMIC] Bash logs detected via program. | Linux Shell Pipe to Shell | `rules/linux/process_creation/proc_creation_lnx_susp_pipe_shell.yml` | 0.66 | bash |
...
| `5002963` | [DYNAMIC] Bash logs detected via program. | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.65 | bash |
...
| `5016159` | [EXTRAHOP] Suspicious User Agent from a Scanner | Renamed PingCastle Binary Execution | `rules/windows/process_creation/proc_creation_win_renamed_pingcastle.yml` | 0.64 | scanner |
...
| `5016159` | [EXTRAHOP] Suspicious User Agent from a Scanner | PUA - PingCastle Execution From Potentially Suspicious | `rules/windows/process_creation/proc_creation_win_pua_pingcastle_script_parent.yml` | 0.63 | scanner |
...
| `5007141` | [WINDOWS-POWERSHELL] Suspicious XOR Command | Suspicious XOR Encoded PowerShell Command | `rules/windows/process_creation/proc_creation_win_powershell_xor_commandline.yml` | 0.63 | bxor, xor, join, char, powershell |
...
| `5003403` | [WINDOWS-SECURITY] A security-enabled global group was | A Member Was Added to a Security-Enabled Global Group | `rules/windows/builtin/security/account_management/win_security_member_added_security_enabled_global_group.yml` | 0.63 | security-enabled, global, group |
...
| `5002963` | [DYNAMIC] Bash logs detected via program. | Linux Base64 Encoded Pipe to Shell | `rules/linux/process_creation/proc_creation_lnx_base64_execution.yml` | 0.62 | bash |
| `5015506` | [WINDOWS-SECURITY] CRITICAL - Microsoft Quick Assist Ex | DNS Query Request By QuickAssist.EXE | `rules/windows/dns_query/dns_query_win_quickassist.yml` | 0.62 | assist, quickassist.exe, quick, microsoft |
| `5003403` | [WINDOWS-SECURITY] A security-enabled global group was | A Member Was Removed From a Security-Enabled Global Gro | `rules/windows/builtin/security/account_management/win_security_member_removed_security_enabled_global_group.yml` | 0.62 | security-enabled, global, group |
...
| `5009295` | [WINDOWS-MISC] Installation of PSEXEC service via Secur | PsExec Service File Creation | `rules/windows/file/file_event/file_event_win_sysinternals_psexec_service.yml` | 0.61 | psexec, installation |
...
| `5002572` | [CYLANCE] Device - Registration | Changes to Device Registration Policy | `rules/cloud/azure/audit_logs/azure_ad_device_registration_policy_changes.yml` | 0.61 | registration, device |
...
| `9870007` | [EXPERIMENTAL][WINDOWS-SECURITY] Kerberos - Service Tic | Suspicious Kerberos RC4 Ticket Encryption | `rules/windows/builtin/security/win_security_susp_rc4_kerberos.yml` | 0.61 | rc4, 0x17, ticket, encryption, kerberos |
...
| `5016503` | [EXTRAHOP] DNS Zone Transfer | Unusual File Download from Direct IP Address | `rules/windows/create_stream_hash/create_stream_hash_susp_ip_domains.yml` | 0.60 | zone |
...
| `5007660` | [DYNAMIC] tcp logs detected via program. | PUA - Ngrok Execution | `rules/windows/process_creation/proc_creation_win_pua_ngrok.yml` | 0.59 | tcp |
| `5100060` | systemd detected | Systemd Service Creation | `rules/linux/auditd/path/lnx_auditd_systemd_service_creation.yml` | 0.59 | systemd |
...
| `5002770` | [FIPAYPIN] Replace macro from outside RFC1918 | Potential Persistence Via PowerShell User Profile Using | `rules/windows/powershell/powershell_script/posh_ps_user_profile_tampering.yml` | 0.58 | value |
...
| `5003015` | [DYNAMIC] MSSQL logs detected via program. | MSSQL XPCmdshell Option Change | `rules/windows/builtin/application/mssqlserver/win_mssql_xp_cmdshell_change.yml` | 0.58 | mssql |
...
| `5100106` | Microsoft MSSQL server detected | MSSQL XPCmdshell Option Change | `rules/windows/builtin/application/mssqlserver/win_mssql_xp_cmdshell_change.yml` | 0.58 | mssql |
...
| `5100019` | Generic crond detected | New Cron File Created | `rules/linux/file_event/file_event_lnx_susp_cron_file_created.yml` | 0.57 | cron |
...
| `5016532` | [EXTRAHOP] TCP SYN Scan | OpenCanary - Host Port Scan (SYN Scan) | `rules/application/opencanary/opencanary_portscan_syn_scan.yml` | 0.56 | syn, scan |
...
| `5100121` | MS-SQL service detected | MSSQL Server Failed Logon | `rules/windows/builtin/application/mssqlserver/win_mssql_failed_logon.yml` | 0.56 | mssql |
...
| `5013880` | [WINDOWS-SYSMON] System Discovery Commands | Windows Webshell Strings | `rules/web/webserver_generic/web_win_webshells_in_access_logs.yml` | 0.56 | tasklist, wmic, get, cmd |
...
| `5003015` | [DYNAMIC] MSSQL logs detected via program. | MSSQL XPCmdshell Suspicious Execution | `rules/windows/builtin/application/mssqlserver/win_mssql_xp_cmdshell_audit_log.yml` | 0.56 | mssql |
...
| `5100106` | Microsoft MSSQL server detected | MSSQL XPCmdshell Suspicious Execution | `rules/windows/builtin/application/mssqlserver/win_mssql_xp_cmdshell_audit_log.yml` | 0.56 | mssql |
| `5100033` | MySQL services detected | OpenCanary - MySQL Login Attempt | `rules/application/opencanary/opencanary_mysql_login_attempt.yml` | 0.55 | mysql |
...
| `5003141` | [ZSCALER] klm123.com Spyware User Agent | Commvault QLogin with PublicSharingUser and GUID Passwo | `rules-emerging-threats/2025/Exploits/CVE-2025-57788/proc_creation_win_exploit_cve_2025_57788.yml` | 0.55 | a-f0-9 |
| `5100125` | Applocker detected | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.55 | applocker |
| `5017356` | [DYNAMIC] SentinelOne Logs Detected | Potential SentinelOne Shell Context Menu Scan Command T | `rules/windows/registry/registry_set/registry_set_sentinelone_shell_context_tampering.yml` | 0.55 | sentinelone |
...
| `5002982` | [DYNAMIC] Linux kernel logs detected via program. | New Kernel Driver Via SC.EXE | `rules/windows/process_creation/proc_creation_win_sc_new_kernel_driver.yml` | 0.54 | kernel |
...
| `9870007` | [EXPERIMENTAL][WINDOWS-SECURITY] Kerberos - Service Tic | Kerberos Network Traffic RC4 Ticket Encryption | `rules/network/zeek/zeek_susp_kerberos_rc4.yml` | 0.54 | rc4, kerberoasting, ticket, encryption, kerberos, request |
...
| `5016392` | [EXTRAHOP] Log4Shell JNDI Injection Attempt by a Scanne | Potential JNDI Injection Exploitation In JVM Based Appl | `rules/application/jvm/java_jndi_injection_exploitation_attempt.yml` | 0.53 | jndi, log4shell, injection |
...
| `5007659` | [DYNAMIC] systemd logs detected via program. | Systemd Service Creation | `rules/linux/auditd/path/lnx_auditd_systemd_service_creation.yml` | 0.53 | systemd |
| `5002999` | [DYNAMIC] Rsync logs detected via program. | Suspicious Invocation of Shell via Rsync | `rules/linux/process_creation/proc_creation_lnx_rsync_shell_spawn.yml` | 0.53 | rsyncd, rsync |
...
| `5007664` | [DYNAMIC] windows applocker logs detected via program. | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.53 | applocker |
...
| `5017964` | [WINDOWS-SECURITY] SMB - Anonymous Access to IPC$ Share | Suspicious Windows ANONYMOUS LOGON Local Account Create | `rules/windows/builtin/security/win_security_susp_local_anon_logon_created.yml` | 0.52 | anonymous, logon |
...
| `5009788` | [WINDOWS-SYSMON] Suspicious WMIC call - computersystem | Computer System Reconnaissance Via Wmic.EXE | `rules/windows/process_creation/proc_creation_win_wmic_recon_computersystem.yml` | 0.52 | computersystem, model, wmic |
...
| `5008384` | [DYNAMIC] windows clipboard logs detected via program. | Clipboard Access Via OSAScript | `rules/macos/process_creation/proc_creation_macos_clipboard_access_via_osascript.yml` | 0.52 | clipboard |
...
| `5017336` | [DYNAMIC] Azure Eventhub Windows MSSQL Logs Detected | MSSQL Server Failed Logon | `rules/windows/builtin/application/mssqlserver/win_mssql_failed_logon.yml` | 0.52 | mssql |
...
| `5100041` | rsync client execution | Suspicious Invocation of Shell via Rsync | `rules/linux/process_creation/proc_creation_lnx_rsync_shell_spawn.yml` | 0.51 | rsync, execution |
...
| `5016468` | [EXTRAHOP] SIP Brute Force | OpenCanary - SIP Request | `rules/application/opencanary/opencanary_sip_request.yml` | 0.51 | sip |
...
| `5015511` | [WINDOWS-SECURITY] Obfuscated IP address Added to Regis | Obfuscated IP Download Activity | `rules/windows/process_creation/proc_creation_win_susp_obfuscated_ip_download.yml` | 0.51 | 0-9, obfuscated, address |
...
| `5013876` | [WINDOWS-SECURITY] Credential Access - Copy NTDS file | Copy From VolumeShadowCopy Via Cmd.EXE | `rules/windows/process_creation/proc_creation_win_cmd_shadowcopy_access.yml` | 0.51 | copy |
...
| `5017578` | [CROWDSTRIKE] Suspicious Execution Detected - Procdump | Potential SysInternals ProcDump Evasion | `rules/windows/process_creation/proc_creation_win_sysinternals_procdump_evasion.yml` | 0.50 | procdump, lsass, dump |
...
| `5002092` | [WINDOWS-APPLOCKER] Allowed an MSI or script to execute | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | 0.50 | applocker, msi, script |
...
## Conceptual candidate, weaker lexical match (review) (1441)
...
| `9870000` | [EXPERIMENTAL][WINDOWS-SECURITY] Authentication - Anony | Suspicious Windows ANONYMOUS LOGON Local Account Create | `rules/windows/builtin/security/win_security_susp_local_anon_logon_created.yml` | 0.50 | anonymous, logon |
...
| `5100030` | Unix 'kernel' messages detected | New Kernel Driver Via SC.EXE | `rules/windows/process_creation/proc_creation_win_sc_new_kernel_driver.yml` | 0.50 | kernel |
...
| `5100189` | Sophos device detected | Tamper With Sophos AV Registry Keys | `rules/windows/registry/registry_set/registry_set_sophos_av_tamper.yml` | 0.50 | sophos |
| `5008380` | [WINDOWS-CLIPBOARD] copy from share drive to local C: c | Copy From VolumeShadowCopy Via Cmd.EXE | `rules/windows/process_creation/proc_creation_win_cmd_shadowcopy_access.yml` | 0.50 | copy |
| `5100192` | Windows PowerShell device detected | PowerShell Scripts Run by a Services | `deprecated/windows/driver_load_win_powershell_script_installed_as_service.yml` | 0.49 | powershell |
...
| `5010825` | [CyberArk] Delete Folder (Has Locked Files) | OneLogin User Account Locked | `rules/identity/onelogin/onelogin_user_account_locked.yml` | 0.49 | locked |
...
| `5100125` | Applocker detected | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | 0.49 | applocker |
...
| `9870004` | [EXPERIMENTAL][WINDOWS-SECURITY] SMB - Anonymous Access | Access To ADMIN$ Network Share | `rules/windows/builtin/security/win_security_admin_share_access.yml` | 0.49 | share, admin |
...
| `5017936` | [WINDOWS-SYSMON] Windows Defender has detected malware | Windows Defender Malware And PUA Scanning Disabled | `rules/windows/builtin/windefend/win_defender_malware_and_pua_scan_disabled.yml` | 0.49 | unwanted, software, defender, potentially, malware |
...
| `5002325` | [BASH] SSH dynamic forwarding | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.49 | bash |
...
| `5016139` | [EXTRAHOP] Microsoft 365 Risky User Activities | Logon from a Risky IP Address | `rules/cloud/m365/threat_management/microsoft365_logon_from_risky_ip_address.yml` | 0.48 | risky, microsoft |
| `5009309` | [WINDOWS-POWERSHELL] Powershell created local user [1/3 | PowerShell Create Local User | `rules/windows/powershell/powershell_script/posh_ps_create_local_user.yml` | 0.48 | new-localuser, local, powershell |
...
| `5016159` | [EXTRAHOP] Suspicious User Agent from a Scanner | Advanced IP Scanner - File Event | `rules/windows/file/file_event/file_event_win_advanced_ip_scanner.yml` | 0.48 | scanner |
...
| `5003105` | [WINDOWS-MISC] CRITICAL - Installation of PSEXEC servic | PsExec Service Installation | `rules/windows/builtin/system/service_control_manager/win_system_service_install_sysinternals_psexec.yml` | 0.48 | psexesvc, psexec, installation |
| `5017576` | [CROWDSTRIKE] Suspicious Execution - Blocked: Procdump | Potential SysInternals ProcDump Evasion | `rules/windows/process_creation/proc_creation_win_sysinternals_procdump_evasion.yml` | 0.48 | procdump, lsass, dump |
...
| `5016391` | [EXTRAHOP] Log4Shell JNDI Injection Attempt | Log4j RCE CVE-2021-44228 Generic | `rules-emerging-threats/2021/Exploits/CVE-2021-44228/web_cve_2021_44228_log4j.yml` | 0.48 | jndi, log4shell |
...
| `5003417` | [WINDOWS-SECURITY] Certificate Services revoked a certi | Certificate Exported From Local Certificate Store | `rules/windows/builtin/certificate_services_client_lifecycle_system/win_certificateservicesclient_lifecycle_system_cert_exported.yml` | 0.48 | certificate |
...
| `5005766` | [DYNAMIC] PowerShell logs detect via program | PowerShell Scripts Run by a Services | `deprecated/windows/driver_load_win_powershell_script_installed_as_service.yml` | 0.48 | powershell |
...
| `5002091` | [WINDOWS-APPLOCKER] Application blocked | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.48 | applocker, blocked |
...
| `5100189` | Sophos device detected | Suspicious Execution of Sc to Delete AV Services | `deprecated/windows/proc_creation_win_sc_delete_av_services.yml` | 0.48 | sophos, device |
...
| `5017456` | [DYNAMIC] Sophos Firewall Logs Detected | Tamper With Sophos AV Registry Keys | `rules/windows/registry/registry_set/registry_set_sophos_av_tamper.yml` | 0.48 | sophos |
...
| `5000008` | [BASH] /bin/sh command line call | Linux Shell Pipe to Shell | `rules/linux/process_creation/proc_creation_lnx_susp_pipe_shell.yml` | 0.48 | bash |
| `5007664` | [DYNAMIC] windows applocker logs detected via program. | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | 0.47 | applocker |
...
| `5016427` | [EXTRAHOP] Anonymous FTP Login | Anonymous IP Address | `rules/cloud/azure/identity_protection/azure_identity_protection_anonymous_ip_address.yml` | 0.47 | anonymous |
...
| `5003417` | [WINDOWS-SECURITY] Certificate Services revoked a certi | CodeIntegrity - Blocked Driver Load With Revoked Certif | `rules/windows/builtin/code_integrity/win_codeintegrity_revoked_driver_blocked.yml` | 0.47 | revoked, certificate |
| `5002982` | [DYNAMIC] Linux kernel logs detected via program. | CodeIntegrity - Revoked Kernel Driver Loaded | `rules/windows/builtin/code_integrity/win_codeintegrity_revoked_driver_loaded.yml` | 0.47 | kernel |
...
| `5100121` | MS-SQL service detected | MSSQL XPCmdshell Option Change | `rules/windows/builtin/application/mssqlserver/win_mssql_xp_cmdshell_change.yml` | 0.47 | mssql |
...
| `5016391` | [EXTRAHOP] Log4Shell JNDI Injection Attempt | Log4j RCE CVE-2021-44228 in Fields | `rules-emerging-threats/2021/Exploits/CVE-2021-44228/web_cve_2021_44228_log4j_fields.yml` | 0.47 | jndi, log4shell |
...
| `5002095` | [WINDOWS-APPLOCKER] Package application allowed | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.47 | applocker, package |
...
| `5100125` | Applocker detected | Possible Applocker Bypass | `deprecated/windows/proc_creation_win_possible_applocker_bypass.yml` | 0.47 | applocker |
...
| `5010329` | [DYNAMIC] MSAPI-Onedrive logs detected via program | Malicious DLL File Dropped in the Teams or OneDrive Fol | `rules/windows/file/file_event/file_event_win_iphlpapi_dll_sideloading.yml` | 0.47 | onedrive |
...
| `5016414` | [EXTRAHOP] SSH Brute Force | Brute Force | `deprecated/other/generic_brute_force.yml` | 0.47 | brute, force |
| `5002097` | [WINDOWS-APPLOCKER] Package application disabled | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.47 | applocker, package |
...
| `5008384` | [DYNAMIC] windows clipboard logs detected via program. | Clipboard Data Collection Via Pbpaste | `rules-threat-hunting/macos/process_creation/proc_creation_macos_pbpaste_execution.yml` | 0.47 | clipboard |
| `5002092` | [WINDOWS-APPLOCKER] Allowed an MSI or script to execute | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.47 | applocker, msi, script |
...
| `5002799` | [WINDOWS-SYSMON] PSExec execution detected | Malicious Driver Load | `rules/windows/driver_load/driver_load_win_mal_drivers.yml` | 0.47 | md5 |
| `5008354` | [WINDOWS-SECURITY] Exfil software rclone detected | Rclone Execution via Command Line or PowerShell | `deprecated/windows/win_susp_rclone_exec.yml` | 0.47 | rclone.exe, rclone, sync, copy, config |
| `5016066` | [EXTRAHOP] BITS Download | BITS Transfer Job Download To Potential Suspicious Fold | `rules/windows/builtin/bits_client/win_bits_client_new_trasnfer_susp_local_folder.yml` | 0.47 | bits, download |
...
| `5009779` | [WINDOWS-SYSMON] PSExec execution detected | Malicious Driver Load | `rules/windows/driver_load/driver_load_win_mal_drivers.yml` | 0.46 | md5 |
...
| `5017965` | [WINDOWS-SECURITY] Kerberos - AS-REP Roasting Attempt w | Suspicious Kerberos RC4 Ticket Encryption | `rules/windows/builtin/security/win_security_susp_rc4_kerberos.yml` | 0.46 | 0x17, ticket, encryption, kerberos, type |
...
| `5016518` | [EXTRAHOP] New LDAP All Object Query Activity | DNS Server Discovery Via LDAP Query | `rules/windows/dns_query/dns_query_win_dns_server_discovery_via_ldap_query.yml` | 0.46 | ldap, query |
...
| `5100141` | Microsoft IIS server detected | HTTP Logging Disabled On IIS Server | `rules/windows/builtin/iis-configuration/win_iis_logging_http_disabled.yml` | 0.46 | iis, server |
...
| `5002999` | [DYNAMIC] Rsync logs detected via program. | Shell Execution via Rsync - Linux | `rules/linux/process_creation/proc_creation_lnx_rsync_shell_execution.yml` | 0.46 | rsyncd, rsync |
...
| `5100097` | su/sudo detected | Linux Sudo Chroot Execution | `rules/linux/process_creation/proc_creation_lnx_chroot_execution.yml` | 0.46 | sudo |
...
| `5016408` | [EXTRAHOP] SMB Brute Force | Brute Force | `deprecated/other/generic_brute_force.yml` | 0.46 | brute, force |
...
| `5009420` | [WINDOWS-SECURITY] Auditing settings on object were cha | Windows Firewall Settings Have Been Changed | `rules/windows/builtin/firewall_as/win_firewall_as_setting_change.yml` | 0.46 | settings, changed |
...
| `5002304` | [BASH] History hiding | Linux Shell Pipe to Shell | `rules/linux/process_creation/proc_creation_lnx_susp_pipe_shell.yml` | 0.46 | bash |
...
| `5008640` | [WINDOWS-CLIPBOARD] Remoe-exec psexec command | PsExec Service File Creation | `rules/windows/file/file_event/file_event_win_sysinternals_psexec_service.yml` | 0.45 | psexec |
...
| `5100141` | Microsoft IIS server detected | Suspicious IIS Module Registration | `rules/windows/process_creation/proc_creation_win_iis_susp_module_registration.yml` | 0.45 | iis, microsoft |
...
| `5017599` | [CROWDSTRIKE] Suspicious Execution Detected- PsExec exe | Renamed PsExec | `deprecated/windows/proc_creation_win_renamed_psexec.yml` | 0.45 | psexec, execution |
...
| `5017578` | [CROWDSTRIKE] Suspicious Execution Detected - Procdump | Procdump Execution | `rules/windows/process_creation/proc_creation_win_sysinternals_procdump.yml` | 0.45 | procdump, execution |
| `5003416` | [WINDOWS-SECURITY] The certificate manager denied a pen | Active Directory Certificate Services Denied Certificat | `rules/windows/builtin/system/microsoft_windows_certification_authority/win_system_adcs_enrollment_request_denied.yml` | 0.45 | denied, certificate, request |
...
| `5100041` | rsync client execution | Shell Execution via Rsync - Linux | `rules/linux/process_creation/proc_creation_lnx_rsync_shell_execution.yml` | 0.45 | rsync, execution |
...
| `5002098` | [WINDOWS-APPLOCKER] Package application installation al | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.45 | applocker, package |
| `5002799` | [WINDOWS-SYSMON] PSExec execution detected | PUA - PingCastle Execution | `rules/windows/process_creation/proc_creation_win_pua_pingcastle.yml` | 0.45 | md5, execution |
| `5007664` | [DYNAMIC] windows applocker logs detected via program. | Possible Applocker Bypass | `deprecated/windows/proc_creation_win_possible_applocker_bypass.yml` | 0.45 | applocker |
...
| `5009779` | [WINDOWS-SYSMON] PSExec execution detected | PUA - PingCastle Execution | `rules/windows/process_creation/proc_creation_win_pua_pingcastle.yml` | 0.45 | md5, execution |
| `5002799` | [WINDOWS-SYSMON] PSExec execution detected | Vulnerable Driver Load | `rules/windows/driver_load/driver_load_win_vuln_drivers.yml` | 0.45 | md5 |
| `5100121` | MS-SQL service detected | MSSQL XPCmdshell Suspicious Execution | `rules/windows/builtin/application/mssqlserver/win_mssql_xp_cmdshell_audit_log.yml` | 0.45 | mssql |
| `5009779` | [WINDOWS-SYSMON] PSExec execution detected | Vulnerable Driver Load | `rules/windows/driver_load/driver_load_win_vuln_drivers.yml` | 0.45 | md5 |
...
| `5007155` | [WINDOWS-POWERSHELL] Cmdlet Scheduled Task Created | Powershell Create Scheduled Task | `rules/windows/powershell/powershell_script/posh_ps_cmdlet_scheduled_task.yml` | 0.45 | new-scheduledtask, ps_scheduledtask, register-scheduledtask, classname |
...
| `5002100` | [WINDOWS-APPLOCKER] Package application installation di | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.45 | applocker, package |
...
| `5002982` | [DYNAMIC] Linux kernel logs detected via program. | CodeIntegrity - Unsigned Kernel Module Loaded | `rules/windows/builtin/code_integrity/win_codeintegrity_unsigned_driver_loaded.yml` | 0.45 | kernel |
...
| `5003420` | [WINDOWS-SECURITY] The certificate manager settings for | Certificate Exported From Local Certificate Store | `rules/windows/builtin/certificate_services_client_lifecycle_system/win_certificateservicesclient_lifecycle_system_cert_exported.yml` | 0.45 | certificate |
...
| `5010586` | [CISCO-SCA] Permissive Amazon Elastic Kubernetes Servic | Azure Kubernetes Cluster Created or Deleted | `rules/cloud/azure/activity_logs/azure_kubernetes_cluster_created_or_deleted.yml` | 0.45 | cluster, kubernetes, created |
...
| `5017456` | [DYNAMIC] Sophos Firewall Logs Detected | Suspicious Execution of Sc to Delete AV Services | `deprecated/windows/proc_creation_win_sc_delete_av_services.yml` | 0.45 | sophos |
...
| `5100189` | Sophos device detected | Potential Tampering With Security Products Via WMIC | `rules/windows/process_creation/proc_creation_win_wmic_uninstall_security_products.yml` | 0.45 | sophos |
...
| `5000005` | [BASH] /etc/shadow access | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.45 | bash |
| `5000011` | [BASH] .bash_history access | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.45 | bash |
| `5002310` | [BASH] PHP subproces execution | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.45 | bash, execution |
| `5014552` | [WINDOWS-SECURITY] Comsrvc MiniDump Command | Process Memory Dump Via Comsvcs.DLL | `rules/windows/process_creation/proc_creation_win_rundll32_process_dump_via_comsvcs.yml` | 0.45 | comsvcs, minidump, rundll32 |
| `5010776` | [CyberArk] Clear User History | Clearing Windows Console History | `rules/windows/powershell/powershell_script/posh_ps_clearing_windows_console_history.yml` | 0.45 | clear, history |
...
| `5016224` | [EXTRAHOP] New Remote Access Software Activity | Detected Windows Software Discovery | `rules/windows/process_creation/proc_creation_win_reg_software_discovery.yml` | 0.44 | software |
| `5017578` | [CROWDSTRIKE] Suspicious Execution Detected - Procdump | Renamed ProcDump Execution | `rules/windows/process_creation/proc_creation_win_renamed_sysinternals_procdump.yml` | 0.44 | procdump, execution |
...
| `5100192` | Windows PowerShell device detected | PowerShell as a Service in Registry | `rules/windows/registry/registry_set/registry_set_powershell_as_service.yml` | 0.44 | powershell |
...
| `5003005` | [DYNAMIC] su/sudo logs detected via program. | Linux Sudo Chroot Execution | `rules/linux/process_creation/proc_creation_lnx_chroot_execution.yml` | 0.44 | sudo |
...
| `5002096` | [WINDOWS-APPLOCKER] Package application audited | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.44 | applocker, package |
...
| `5002954` | [WINDOWS-MISC] Event log has been cleared. | Eventlog Cleared | `rules/windows/builtin/system/microsoft_windows_eventlog/win_system_eventlog_cleared.yml` | 0.44 | eventlog, cleared |
| `5016464` | [EXTRAHOP] New WMI Method Launch Activity | WMI Event Subscription | `rules/windows/wmi_event/sysmon_wmi_event_subscription.yml` | 0.43 | wmi, method |
| `5002989` | [DYNAMIC] OpenVPN logs detected via program. | Suspicious Application Installed | `rules/windows/builtin/shell_core/win_shell_core_susp_packages_installed.yml` | 0.43 | openvpn |
...
| `5013747` | [NETWRIX] Logon Activity - Possible Brute Force Attempt | Suspicious Windows ANONYMOUS LOGON Local Account Create | `rules/windows/builtin/security/win_security_susp_local_anon_logon_created.yml` | 0.43 | logon |
...
| `5100030` | Unix 'kernel' messages detected | CodeIntegrity - Revoked Kernel Driver Loaded | `rules/windows/builtin/code_integrity/win_codeintegrity_revoked_driver_loaded.yml` | 0.43 | kernel |
...
| `5100097` | su/sudo detected | Sudo Privilege Escalation CVE-2019-14287 | `rules-emerging-threats/2019/Exploits/CVE-2019-14287/proc_creation_lnx_exploit_cve_2019_14287.yml` | 0.43 | sudo |
...
| `5005766` | [DYNAMIC] PowerShell logs detect via program | PowerShell as a Service in Registry | `rules/windows/registry/registry_set/registry_set_powershell_as_service.yml` | 0.43 | powershell |
...
| `5100192` | Windows PowerShell device detected | PowerShell Scripts Installed as Services - Security | `rules/windows/builtin/security/win_security_powershell_script_installed_as_service.yml` | 0.43 | powershell |
...
| `5017456` | [DYNAMIC] Sophos Firewall Logs Detected | Potential Tampering With Security Products Via WMIC | `rules/windows/process_creation/proc_creation_win_wmic_uninstall_security_products.yml` | 0.43 | sophos |
...
| `5017964` | [WINDOWS-SECURITY] SMB - Anonymous Access to IPC$ Share | Anonymous IP Address | `rules/cloud/azure/identity_protection/azure_identity_protection_anonymous_ip_address.yml` | 0.43 | anonymous |
...
| `5016392` | [EXTRAHOP] Log4Shell JNDI Injection Attempt by a Scanne | Log4j RCE CVE-2021-44228 Generic | `rules-emerging-threats/2021/Exploits/CVE-2021-44228/web_cve_2021_44228_log4j.yml` | 0.43 | jndi, log4shell |
...
| `5100141` | Microsoft IIS server detected | IIS WebServer Log Deletion via CommandLine Utilities | `rules/windows/process_creation/proc_creation_win_iis_logs_deletion.yml` | 0.43 | iis |
...
| `5002565` | [BASH] root password change attempt | Linux Base64 Encoded Pipe to Shell | `rules/linux/process_creation/proc_creation_lnx_base64_execution.yml` | 0.43 | bash |
| `5002311` | [BASH] Perl socket execution | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.43 | bash, execution |
...
| `991015` | [AWS] EC2 Create Snapshot | AWS Snapshot Backup Exfiltration | `rules/cloud/aws/cloudtrail/aws_snapshot_backup_exfiltration.yml` | 0.43 | snapshot, ec2, ec2.amazonaws.com, aws |
| `5016427` | [EXTRAHOP] Anonymous FTP Login | COLDSTEEL RAT Anonymous User Process Execution | `rules-emerging-threats/2023/Malware/COLDSTEEL/proc_creation_win_malware_coldsteel_anonymous_process.yml` | 0.42 | anonymous |
...
| `5016224` | [EXTRAHOP] New Remote Access Software Activity | Detected Windows Software Discovery - PowerShell | `rules/windows/powershell/powershell_script/posh_ps_software_discovery.yml` | 0.42 | software |
...
| `5008089` | [WINDOWS-SYSMON] Possible DLL Hijacking of vssapi.dll | Suspicious Volume Shadow Copy Vssapi.dll Load | `rules/windows/image_load/image_load_dll_vssapi_susp_load.yml` | 0.42 | vssapi.dll, dll |
...
| `5002099` | [WINDOWS-APPLOCKER] Package application installation au | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.42 | applocker, package |
...
| `5010154` | [WINDOWS-SYSMON] Possible DLL Hijacking of vssapi.dll | Suspicious Volume Shadow Copy Vssapi.dll Load | `rules/windows/image_load/image_load_dll_vssapi_susp_load.yml` | 0.42 | vssapi.dll, dll |
...
| `5008090` | [WINDOWS-SYSMON] Possible DLL Hijacking of vsstrace.dll | Potentially Suspicious Volume Shadow Copy Vsstrace.dll | `rules/windows/image_load/image_load_dll_vsstrace_susp_load.yml` | 0.42 | vsstrace.dll, dll |
| `5002091` | [WINDOWS-APPLOCKER] Application blocked | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.42 | applocker, blocked |
...
| `5010155` | [WINDOWS-SYSMON] Possible DLL Hijacking of vsstrace.dll | Potentially Suspicious Volume Shadow Copy Vsstrace.dll | `rules/windows/image_load/image_load_dll_vsstrace_susp_load.yml` | 0.42 | vsstrace.dll, dll |
...
| `5011427` | [DYNAMIC] Sophos Aetas logs detected via program. | Tamper With Sophos AV Registry Keys | `rules/windows/registry/registry_set/registry_set_sophos_av_tamper.yml` | 0.42 | sophos |
| `5003420` | [WINDOWS-SECURITY] The certificate manager settings for | Windows Firewall Settings Have Been Changed | `rules/windows/builtin/firewall_as/win_firewall_as_setting_change.yml` | 0.42 | settings, changed |
...
| `5003417` | [WINDOWS-SECURITY] Certificate Services revoked a certi | CodeIntegrity - Blocked Image Load With Revoked Certifi | `rules/windows/builtin/code_integrity/win_codeintegrity_revoked_image_blocked.yml` | 0.42 | revoked, certificate |
...
| `5016392` | [EXTRAHOP] Log4Shell JNDI Injection Attempt by a Scanne | Log4j RCE CVE-2021-44228 in Fields | `rules-emerging-threats/2021/Exploits/CVE-2021-44228/web_cve_2021_44228_log4j_fields.yml` | 0.42 | jndi, log4shell |
...
| `5003407` | [WINDOWS-SECURITY] A security-enabled universal group w | A Security-Enabled Global Group Was Deleted | `rules/windows/builtin/security/account_management/win_security_security_enabled_global_group_deleted.yml` | 0.42 | security-enabled, group |
...
| `5005766` | [DYNAMIC] PowerShell logs detect via program | PowerShell Scripts Installed as Services - Security | `rules/windows/builtin/security/win_security_powershell_script_installed_as_service.yml` | 0.42 | powershell |
...
| `5100160` | AWS GuardDuty detected | AWS GuardDuty Important Change | `rules/cloud/aws/cloudtrail/aws_guardduty_disruption.yml` | 0.42 | guardduty.amazonaws.com, guardduty, aws |
...
| `5002313` | [BASH] Ruby socket execution | Linux Shell Pipe to Shell | `rules/linux/process_creation/proc_creation_lnx_susp_pipe_shell.yml` | 0.42 | bash |
...
| `5100097` | su/sudo detected | Sudo Privilege Escalation CVE-2019-14287 - Builtin | `rules-emerging-threats/2019/Exploits/CVE-2019-14287/lnx_sudo_exploit_cve_2019_14287.yml` | 0.41 | sudo |
...
| `5011302` | [CARBONBLACK-APP-CONTROL] Multiple failed logins (Warni | Multiple Users Failing to Authenticate from Single Proc | `unsupported/windows/win_security_susp_failed_logons_single_process.yml` | 0.41 | logins, multiple, failed |
...
| `5003105` | [WINDOWS-MISC] CRITICAL - Installation of PSEXEC servic | PsExec Service File Creation | `rules/windows/file/file_event/file_event_win_sysinternals_psexec_service.yml` | 0.41 | psexec, installation |
...
| `5016520` | [EXTRAHOP] New LDAP GPO Query Activity | DNS Server Discovery Via LDAP Query | `rules/windows/dns_query/dns_query_win_dns_server_discovery_via_ldap_query.yml` | 0.41 | ldap, query |
| `5017577` | [CROWDSTRIKE] Suspicious Execution - Killed: Procdump l | Procdump Execution | `rules/windows/process_creation/proc_creation_win_sysinternals_procdump.yml` | 0.41 | procdump, execution |
...
| `5003005` | [DYNAMIC] su/sudo logs detected via program. | Sudo Privilege Escalation CVE-2019-14287 | `rules-emerging-threats/2019/Exploits/CVE-2019-14287/proc_creation_lnx_exploit_cve_2019_14287.yml` | 0.41 | sudo |
...
| `5100030` | Unix 'kernel' messages detected | CodeIntegrity - Unsigned Kernel Module Loaded | `rules/windows/builtin/code_integrity/win_codeintegrity_unsigned_driver_loaded.yml` | 0.41 | kernel |
...
| `5011414` | [DYNAMIC] Bonding logs detected via program. | New Kernel Driver Via SC.EXE | `rules/windows/process_creation/proc_creation_win_sc_new_kernel_driver.yml` | 0.41 | kernel |
...
| `5000925` | [FORTINET] License about to expired | Invalid PIM License | `rules/cloud/azure/privileged_identity_management/azure_pim_invalid_license.yml` | 0.41 | license |
...
| `5005539` | [CLOUDTRAIL] RDS cloudtrail event detected - (ModifyDBI | AWS RDS Master Password Change | `rules/cloud/aws/cloudtrail/aws_rds_change_master_password.yml` | 0.41 | modifydbinstance, rds.amazonaws.com, rds |
...
| `5016210` | [EXTRAHOP] Metasploit C&C TLS Connection | Metasploit SMB Authentication | `rules/windows/builtin/security/win_security_metasploit_authentication.yml` | 0.41 | metasploit |
| `5000056` | [SYSLOG] Kernel TCP/IP redirect attempt | Suspicious RDP Redirect Using TSCON | `rules/windows/process_creation/proc_creation_win_tscon_rdp_redirect.yml` | 0.41 | redirect |
| `5003111` | [NXLOG] Missing Windows Log Message | Obfuscated IP Via CLI | `rules/windows/process_creation/proc_creation_win_susp_obfuscated_ip_via_cli.yml` | 0.41 | 0-9 |
...
| `5007370` | [SOPHOS] Detected malware | Tamper With Sophos AV Registry Keys | `rules/windows/registry/registry_set/registry_set_sophos_av_tamper.yml` | 0.41 | endpoint, sophos |
| `5007388` | [SOPHOS] Malware detected | Tamper With Sophos AV Registry Keys | `rules/windows/registry/registry_set/registry_set_sophos_av_tamper.yml` | 0.41 | endpoint, sophos |
| `5008442` | [WINDOWS-AUTH] A member was added to a security-enabled | A Security-Enabled Global Group Was Deleted | `rules/windows/builtin/security/account_management/win_security_security_enabled_global_group_deleted.yml` | 0.41 | security-enabled, group |
| `5014312` | [DYNAMIC] JAMF Protect logs detected via program. | JAMF MDM Potential Suspicious Child Process | `rules/macos/process_creation/proc_creation_macos_jamf_susp_child.yml` | 0.41 | jamf |
| `5016345` | [EXTRAHOP] CVE-2022-22954 VMware Workspace ONE Access a | CVE-2022-31659 VMware Workspace ONE Access RCE | `rules-emerging-threats/2022/Exploits/CVE-2022-31659/web_cve_2022_31659_vmware_rce.yml` | 0.41 | workspace, vmware, one |
...
| `5015850` | [MICROSOFT_DEFENDER_ENDPOINT] Potentially Unwanted Soft | Windows Defender Malware And PUA Scanning Disabled | `rules/windows/builtin/windefend/win_defender_malware_and_pua_scan_disabled.yml` | 0.40 | unwanted, pua, software, potentially |
| `5015851` | [MICROSOFT_DEFENDER_ENDPOINT] Potentially Unwanted Soft | Windows Defender Malware And PUA Scanning Disabled | `rules/windows/builtin/windefend/win_defender_malware_and_pua_scan_disabled.yml` | 0.40 | unwanted, pua, software, potentially |
| `5005958` | [WEB-ATTACKS] Log4j exploit attempt - CVE-2021-44228 | Log4j RCE CVE-2021-44228 Generic | `rules-emerging-threats/2021/Exploits/CVE-2021-44228/web_cve_2021_44228_log4j.yml` | 0.40 | cve-2021-44228, jndi, log4j |
...
| `5014395` | [FORTINET] License about to expired | Invalid PIM License | `rules/cloud/azure/privileged_identity_management/azure_pim_invalid_license.yml` | 0.40 | license |
| `5017577` | [CROWDSTRIKE] Suspicious Execution - Killed: Procdump l | Renamed ProcDump Execution | `rules/windows/process_creation/proc_creation_win_renamed_sysinternals_procdump.yml` | 0.40 | procdump, execution |
| `5100053` | sshd detected | Potential Exploitation of CVE-2024-3094 - Suspicious SS | `rules-emerging-threats/2024/Exploits/CVE-2024-3094/proc_creation_lnx_exploit_cve_2024_3094_sshd_child_process.yml` | 0.40 | sshd |
...
| `5003416` | [WINDOWS-SECURITY] The certificate manager denied a pen | Certificate Exported From Local Certificate Store | `rules/windows/builtin/certificate_services_client_lifecycle_system/win_certificateservicesclient_lifecycle_system_cert_exported.yml` | 0.40 | certificate |
...
| `5005959` | [WEB-ATTACKS] Log4j exploit attempt via hex encoding - | Log4j RCE CVE-2021-44228 Generic | `rules-emerging-threats/2021/Exploits/CVE-2021-44228/web_cve_2021_44228_log4j.yml` | 0.40 | cve-2021-44228, jndi, log4j |
...
| `5002095` | [WINDOWS-APPLOCKER] Package application allowed | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.40 | applocker |
...
| `5002097` | [WINDOWS-APPLOCKER] Package application disabled | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.40 | applocker |
| `5003005` | [DYNAMIC] su/sudo logs detected via program. | Sudo Privilege Escalation CVE-2019-14287 - Builtin | `rules-emerging-threats/2019/Exploits/CVE-2019-14287/lnx_sudo_exploit_cve_2019_14287.yml` | 0.40 | sudo |
...
| `5004767` | [AZURE-EVENTHUB-AD] Risk Event Detected - investigation | Azure AD Threat Intelligence | `rules/cloud/azure/identity_protection/azure_identity_protection_threat_intel.yml` | 0.40 | investigationsthreatintelligence |
...
| `` | Aggregate of rules setting Sagan bit recon | Curl Web Request With Potential Custom User-Agent | `rules/windows/process_creation/proc_creation_win_curl_custom_user_agent.yml` | 0.40 | user-agent |
...
| `5015084` | [WINDOWS-POWERSHELL] Windows Defender Uninstalled via P | Renamed Powershell Under Powershell Channel | `rules/windows/powershell/powershell_classic/posh_pc_renamed_powershell.yml` | 0.40 | hostapplication, powershell |
| `5016218` | [EXTRAHOP] New Local DNS Server Activity | DNS Server Error Failed Loading the ServerLevelPluginDL | `rules/windows/builtin/dns_server/win_dns_server_susp_server_level_plugin_dll.yml` | 0.40 | dns, server |
...
| `5008639` | [WINDOWS-CLIPBOARD] Uninstall Windows Defender Command | Suspicious Uninstall of Windows Defender Feature via Po | `rules/windows/process_creation/proc_creation_win_powershell_uninstall_defender_feature.yml` | 0.39 | uninstall-windowsfeature, windows-defender, uninstall, defender |
...
| `5016069` | [EXTRAHOP] New Remote System Shutdown Attempt | Potential KamiKakaBot Activity - Shutdown Schedule Task | `rules-emerging-threats/2024/Malware/KamiKakaBot/proc_creation_win_malware_kamikakabot_schtasks_persistence.yml` | 0.39 | shutdown |
...
| `5008384` | [DYNAMIC] windows clipboard logs detected via program. | Clipboard Collection with Xclip Tool - Auditd | `rules/linux/auditd/execve/lnx_auditd_clipboard_collection.yml` | 0.39 | clipboard |
...
| `5011427` | [DYNAMIC] Sophos Aetas logs detected via program. | Suspicious Execution of Sc to Delete AV Services | `deprecated/windows/proc_creation_win_sc_delete_av_services.yml` | 0.39 | sophos |
...
| `5010582` | [CISCO-SCA] Non-Service Port Scanner | Renamed PingCastle Binary Execution | `rules/windows/process_creation/proc_creation_win_renamed_pingcastle.yml` | 0.39 | scanner |
...
| `5017569` | [CROWDSTRIKE] Suspicious Execution Detected - PsExec ex | PsExec Service File Creation | `rules/windows/file/file_event/file_event_win_sysinternals_psexec_service.yml` | 0.39 | psexec, execution |
| `5100016` | csh shell in use | Potentially Suspicious Shell Script Creation in Profile | `rules/linux/file_event/file_event_lnx_susp_shell_script_under_profile_directory.yml` | 0.39 | csh, shell |
...
| `5017964` | [WINDOWS-SECURITY] SMB - Anonymous Access to IPC$ Share | Activity from Anonymous IP Addresses | `rules/cloud/m365/threat_management/microsoft365_activity_from_anonymous_ip_addresses.yml` | 0.39 | anonymous, security |
...
| `5009354` | [WINDOWS-POWERSHELL] Suspicious XOR Command | Potential Xor Encoded PowerShell Command | `deprecated/windows/proc_creation_win_powershell_xor_encoded_command.yml` | 0.39 | bxor, xor, powershell |
...
| `5007126` | [WINDOWS-POWERSHELL] Microsoft Defender Security Regist | Disable Windows Defender Functionalities Via Registry K | `rules/windows/registry/registry_set/registry_set_windows_defender_tamper.yml` | 0.39 | disableintrusionpreventionsystem, disableioavprotection, disablebehavi |
| `5010582` | [CISCO-SCA] Non-Service Port Scanner | PUA - PingCastle Execution From Potentially Suspicious | `rules/windows/process_creation/proc_creation_win_pua_pingcastle_script_parent.yml` | 0.39 | scanner |
...
| `5014548` | [WINDOWS-SECURITY] Fax service installed - Possible Bla | Change the Fax Dll | `rules/windows/registry/registry_set/registry_set_fax_dll_persistance.yml` | 0.39 | fax |
...
| `5009339` | [WINDOWS-POWERSHELL] Microsoft Defender Security Regist | Disable Windows Defender Functionalities Via Registry K | `rules/windows/registry/registry_set/registry_set_windows_defender_tamper.yml` | 0.39 | disableintrusionpreventionsystem, disableioavprotection, disablebehavi |
...
| `5008358` | [WINDOWS-SECURITY] A service was installed in the syste | IcedID Malware Suspicious Single Digit DLL Execution Vi | `rules-emerging-threats/2023/Malware/IcedID/proc_creation_win_malware_icedid_rundll32_dllregisterserver.yml` | 0.39 | dllregisterserver, rundll32 |
| `5002098` | [WINDOWS-APPLOCKER] Package application installation al | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.38 | applocker |
...
| `5008415` | [WINDOWS-APPLOCKER] Package application allowed | Deployment AppX Package Was Blocked By AppLocker | `rules/windows/builtin/appxdeployment_server/win_appxdeployment_server_applocker_block.yml` | 0.38 | applocker, package |
| `5008369` | [WINDOWS-CLIPBOARD] Remoe-exec psexec command | Renamed PsExec | `deprecated/windows/proc_creation_win_renamed_psexec.yml` | 0.38 | psexec |
...
| `5016162` | [EXTRAHOP] Unusual Anonymous FTP Login | COLDSTEEL RAT Anonymous User Process Execution | `rules-emerging-threats/2023/Malware/COLDSTEEL/proc_creation_win_malware_coldsteel_anonymous_process.yml` | 0.38 | anonymous |
...
| `5016404` | [EXTRAHOP] Responder NTLM Activity | NTLM Logon | `rules/windows/builtin/ntlm/win_susp_ntlm_auth.yml` | 0.38 | ntlm |
...
| `5002100` | [WINDOWS-APPLOCKER] Package application installation di | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.38 | applocker |
...
| `5008640` | [WINDOWS-CLIPBOARD] Remoe-exec psexec command | Renamed PsExec | `deprecated/windows/proc_creation_win_renamed_psexec.yml` | 0.38 | psexec |
...
| `5013569` | [WINDOWS-SECURITY] Log on using Non-Standard Workstatio | Locked Workstation | `rules/windows/builtin/security/win_security_workstation_was_locked.yml` | 0.38 | workstation |
...
| `5016514` | [EXTRAHOP] LDAP SPN Scan | Potential SPN Enumeration Via Setspn.EXE | `rules/windows/process_creation/proc_creation_win_setspn_spn_enumeration.yml` | 0.38 | spn |
...
| `5100180` | Github device detected | New Github Organization Member Added | `rules/application/github/audit/github_new_org_member.yml` | 0.38 | github |
...
| `5002770` | [FIPAYPIN] Replace macro from outside RFC1918 | Disabled IE Security Features | `rules/windows/process_creation/proc_creation_win_powershell_disable_ie_features.yml` | 0.38 | value |
...
| `5010627` | [CISCO-SCA] Vulnerable Transport Security Protocol | MSExchange Transport Agent Installation - Builtin | `rules/windows/builtin/msexchange/win_exchange_transportagent.yml` | 0.38 | transport |
...
| `5001083` | [SONICWALL] Possible TCP Port Scan | Bpfdoor TCP Ports Redirect | `rules/linux/auditd/execve/lnx_auditd_bpfdoor_port_redirect.yml` | 0.38 | tcp, port |
...
| `5011421` | [DYNAMIC] Github logs detected via program. | New Github Organization Member Added | `rules/application/github/audit/github_new_org_member.yml` | 0.38 | github |
...
| `5002988` | [DYNAMIC] Nginx logs detected via program. | Nginx Core Dump | `rules/web/product/nginx/web_nginx_core_dump.yml` | 0.38 | nginx |
| `5011427` | [DYNAMIC] Sophos Aetas logs detected via program. | Potential Tampering With Security Products Via WMIC | `rules/windows/process_creation/proc_creation_win_wmic_uninstall_security_products.yml` | 0.38 | sophos |
...
| `5016206` | [EXTRAHOP] Koadic C&C Implant Activity | iOS Implant URL Pattern | `deprecated/web/proxy_ios_implant.yml` | 0.38 | implant |
...
| `5000157` | [APACHE] Attempt to access forbidden directory index | Removal Of Index Value to Hide Schedule Task - Registry | `rules/windows/registry/registry_delete/registry_delete_schtasks_hide_task_via_index_value_removal.yml` | 0.38 | index |
| `5000933` | [FORTINET] Access profile changed | PowerShell Profile Modification | `rules/windows/file/file_event/file_event_win_susp_powershell_profile.yml` | 0.38 | profile |
| `5010601` | [CISCO-SCA] Role Violation | User Added to an Administrator's Azure AD Role | `rules/cloud/azure/audit_logs/azure_ad_user_added_to_admin_role.yml` | 0.38 | role |
| `5002333` | [BASH] LD_PRELOAD environment variable access | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.38 | bash |
| `5002334` | [BASH] LD_LIBRARY_PATH environment variable access | Bash Interactive Shell | `rules/linux/process_creation/proc_creation_lnx_bash_interactive_shell.yml` | 0.38 | bash |
...
| `5010809` | [CyberArk] Clear Expired History | Clearing Windows Console History | `rules/windows/powershell/powershell_script/posh_ps_clearing_windows_console_history.yml` | 0.38 | clear, history |
...
| `5010627` | [CISCO-SCA] Vulnerable Transport Security Protocol | Failed MSExchange Transport Agent Installation | `rules/windows/builtin/msexchange/win_exchange_transportagent_failed.yml` | 0.37 | transport |
...
| `5016224` | [EXTRAHOP] New Remote Access Software Activity | Remote Access Tool - ScreenConnect Execution | `rules/windows/process_creation/proc_creation_win_remote_access_tools_screenconnect.yml` | 0.37 | software, remote |
| `5017563` | [CROWDSTRIKE] Possible Defense Evasion Attempt Detected | Potential Defense Evasion Activity Via Emoji Usage In C | `rules/windows/process_creation/proc_creation_win_susp_emoji_usage_in_cli_1.yml` | 0.37 | defense, evasion |
| `5017563` | [CROWDSTRIKE] Possible Defense Evasion Attempt Detected | Potential Defense Evasion Activity Via Emoji Usage In C | `rules/windows/process_creation/proc_creation_win_susp_emoji_usage_in_cli_2.yml` | 0.37 | defense, evasion |
| `5017563` | [CROWDSTRIKE] Possible Defense Evasion Attempt Detected | Potential Defense Evasion Activity Via Emoji Usage In C | `rules/windows/process_creation/proc_creation_win_susp_emoji_usage_in_cli_3.yml` | 0.37 | defense, evasion |
...
| `5013880` | [WINDOWS-SYSMON] System Discovery Commands | Suspicious Service Path Modification | `rules/windows/process_creation/proc_creation_win_sc_service_path_modification.yml` | 0.37 | cmd |
| `5016400` | [EXTRAHOP] RDP Brute Force | Mstsc.EXE Execution With Local RDP File | `rules/windows/process_creation/proc_creation_win_mstsc_run_local_rdp_file.yml` | 0.37 | rdp |
...
| `5010502` | [CISCO-SCA] Anomalous Windows Workstation | Locked Workstation | `rules/windows/builtin/security/win_security_workstation_was_locked.yml` | 0.37 | workstation |
| `5010582` | [CISCO-SCA] Non-Service Port Scanner | PUA - Advanced Port Scanner Execution | `rules/windows/process_creation/proc_creation_win_pua_advanced_port_scanner.yml` | 0.37 | scanner, port |
...
| `5010818` | [CyberArk] Get User's Details | Bitbucket User Permissions Export Attempt | `rules/application/bitbucket/audit/bitbucket_audit_user_permissions_export_attempt_detected.yml` | 0.37 | details |
| `5010819` | [CyberArk] Get Your User's Details | Bitbucket User Permissions Export Attempt | `rules/application/bitbucket/audit/bitbucket_audit_user_permissions_export_attempt_detected.yml` | 0.37 | details |
...
| `5015849` | [MICROSOFT_DEFENDER_ENDPOINT] Potentially Unwanted Soft | Windows Defender Malware And PUA Scanning Disabled | `rules/windows/builtin/windefend/win_defender_malware_and_pua_scan_disabled.yml` | 0.37 | unwanted, pua, software, potentially |
...
| `5013571` | [WINDOWS-SECURITY] Log on using Non-Standard Workstatio | Locked Workstation | `rules/windows/builtin/security/win_security_workstation_was_locked.yml` | 0.37 | workstation |
...
| `5013855` | [WINDOWS-SYSMON] Windows Defender has detected malware | Windows Defender Malware And PUA Scanning Disabled | `rules/windows/builtin/windefend/win_defender_malware_and_pua_scan_disabled.yml` | 0.37 | unwanted, software, defender, potentially, malware |
...
| `5001126` | [ADTRAN] TCP INTERNAL BLOCK | PUA - Ngrok Execution | `rules/windows/process_creation/proc_creation_win_pua_ngrok.yml` | 0.37 | tcp |
...
| `5017937` | [WINDOWS-SYSMON] Windows Defender has detected potentia | Windows Defender Malware And PUA Scanning Disabled | `rules/windows/builtin/windefend/win_defender_malware_and_pua_scan_disabled.yml` | 0.37 | unwanted, software, defender, potentially |
...
| `5000111` | [CISCO-IOS] IOS configuration changed | PowerShell Console History Logs Deleted | `rules/windows/file/file_delete/file_delete_win_delete_powershell_command_history.yml` | 0.37 | console |
...
| `5002638` | [SONICWALL] VPN PKI - Bad CRL format | XSL Script Execution Via WMIC.EXE | `rules/windows/process_creation/proc_creation_win_wmic_xsl_script_processing.yml` | 0.36 | format |
...
| `5015508` | [WINDOWS-SECURITY] Suspicious Tasklist Command Detected | Suspicious Tasklist Discovery Command | `rules-threat-hunting/windows/process_creation/proc_creation_win_tasklist_basic_execution.yml` | 0.36 | tasklist |
...
| `5003105` | [WINDOWS-MISC] CRITICAL - Installation of PSEXEC servic | PsExec Default Named Pipe | `rules-threat-hunting/windows/pipe_created/pipe_created_sysinternals_psexec_default_pipe.yml` | 0.36 | psexesvc, psexec |
...
| `5100034` | nginx web server running | Nginx Core Dump | `rules/web/product/nginx/web_nginx_core_dump.yml` | 0.36 | nginx |
...
| `5100161` | AWS IAM detected | AWS IAM Backdoor Users Keys | `rules/cloud/aws/cloudtrail/aws_iam_backdoor_users_keys.yml` | 0.36 | iam.amazonaws.com, iam, aws |
...
| `5003407` | [WINDOWS-SECURITY] A security-enabled universal group w | A Member Was Added to a Security-Enabled Global Group | `rules/windows/builtin/security/account_management/win_security_member_added_security_enabled_global_group.yml` | 0.36 | security-enabled, group |
...
| `5014633` | [LINUX-SECURITY] Suspicious Command (/proc/<PID>/cmdlin | Container Residence Discovery Via Proc Virtual FS | `rules/linux/process_creation/proc_creation_lnx_susp_container_residence_discovery.yml` | 0.36 | proc, cat |
...
| `5002090` | [WINDOWS-APPLOCKER] Allowed program to execute | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | 0.36 | applocker |
...
| `9870000` | [EXPERIMENTAL][WINDOWS-SECURITY] Authentication - Anony | Pass the Hash Activity 2 | `rules/windows/builtin/security/account_management/win_security_pass_the_hash_2.yml` | 0.36 | ntlmssp, anonymous, logon |
...
| `5100057` | Generic syslog service detected | ESXi Syslog Configuration Change Via ESXCLI | `rules/linux/process_creation/proc_creation_lnx_esxcli_syslog_config_change.yml` | 0.36 | syslog |
...
| `5003420` | [WINDOWS-SECURITY] The certificate manager settings for | Active Directory Certificate Services Denied Certificat | `rules/windows/builtin/system/microsoft_windows_certification_authority/win_system_adcs_enrollment_request_denied.yml` | 0.36 | certificate |
...
| `5007615` | [DYNAMIC] adtran logs detected via program. | FortiGate - New Firewall Policy Added | `rules/network/fortinet/fortigate/fortinet_fortigate_new_firewall_policy_added.yml` | 0.36 | firewall |
...
| `5002095` | [WINDOWS-APPLOCKER] Package application allowed | AppLocker Prevented Application or Script from Running | `rules/windows/builtin/applocker/win_applocker_application_was_prevented_from_running.yml` | 0.36 | applocker |
...
| `5000111` | [CISCO-IOS] IOS configuration changed | Okta New Admin Console Behaviours | `rules/identity/okta/okta_new_behaviours_admin_console.yml` | 0.36 | console |
...
| `5000996` | [SNORT] A TCP connection was detected | PUA - Ngrok Execution | `rules/windows/process_creation/proc_creation_win_pua_ngrok.yml` | 0.36 | tcp |
...
| `5016496` | [EXTRAHOP] Alias Member Enumeration Attempt | Potential PowerShell Obfuscation Using Character Join | `rules/windows/powershell/powershell_script/posh_ps_susp_alias_obfscuation.yml` | 0.36 | alias |
...
| `5100157` | AWS Cloudtrail detected | AWS CloudTrail Important Change | `rules/cloud/aws/cloudtrail/aws_cloudtrail_disable_logging.yml` | 0.36 | cloudtrail.amazonaws.com, aws, cloudtrail |
...
| `5014479` | [FORTINET] VPN Login After Brute Force | Brute Force | `deprecated/other/generic_brute_force.yml` | 0.36 | brute, force |
...
| `5002094` | [WINDOWS-APPLOCKER] Prevent MSI/Script to execute | AppLocker Application Would Have Been Blocked | `rules/windows/builtin/applocker/win_applocker_application_would_have_been_blocked.yml` | 0.36 | applocker |
...
| `5014640` | [LINUX-SECURITY] User Added to SUDO Group Command Detec | Sudo Privilege Escalation CVE-2019-14287 | `rules-emerging-threats/2019/Exploits/CVE-2019-14287/proc_creation_lnx_exploit_cve_2019_14287.yml` | 0.36 | sudo |
...
| `5011414` | [DYNAMIC] Bonding logs detected via program. | CodeIntegrity - Revoked Kernel Driver Loaded | `rules/windows/builtin/code_integrity/win_codeintegrity_revoked_driver_loaded.yml` | 0.36 | kernel |
...
| `5007403` | [SOPHOS] PUA cleaned up | Potential Tampering With Security Products Via WMIC | `rules/windows/process_creation/proc_creation_win_wmic_uninstall_security_products.yml` | 0.36 | endpoint, sophos |
...
| `5003407` | [WINDOWS-SECURITY] A security-enabled universal group w | A Member Was Removed From a Security-Enabled Global Gro | `rules/windows/builtin/security/account_management/win_security_member_removed_security_enabled_global_group.yml` | 0.36 | security-enabled, group |
...
| `5010520` | [CISCO-SCA] AWS Temporary Token Persistence | Anomalous Token | `rules/cloud/azure/identity_protection/azure_identity_protection_anomalous_token.yml` | 0.35 | token |
...
| `5003764` | [WINDOWS-SECURITY] An attempt was made to set the Direc | Directory Service Restore Mode(DSRM) Registry Value Tam | `rules/windows/registry/registry_set/registry_set_dsrm_tampering.yml` | 0.35 | restore, mode, administrator, password, set, directory |
...
| `5100019` | Generic crond detected | Scheduled Cron Task/Job - Linux | `rules/linux/process_creation/proc_creation_lnx_schedule_task_job_cron.yml` | 0.35 | cron |
| `5010455` | [DYNAMIC] AWS GuardDuty logs detected via program | AWS GuardDuty Important Change | `rules/cloud/aws/cloudtrail/aws_guardduty_disruption.yml` | 0.35 | guardduty, aws |
| `5010881` | [CyberArk] Changes to the Master Policy failed | AWS RDS Master Password Change | `rules/cloud/aws/cloudtrail/aws_rds_change_master_password.yml` | 0.35 | master |
...
| `5005960` | [APACHE] Log4j exploit attempt - CVE-2021-44228 | Log4j RCE CVE-2021-44228 Generic | `rules-emerging-threats/2021/Exploits/CVE-2021-44228/web_cve_2021_44228_log4j.yml` | 0.35 | cve-2021-44228, jndi, log4j |
...
| `5013880` | [WINDOWS-SYSMON] System Discovery Commands | Suspicious RunAs-Like Flag Combination | `rules/windows/process_creation/proc_creation_win_susp_privilege_escalation_cli_patterns.yml` | 0.35 | cmd |
...
| `5007615` | [DYNAMIC] adtran logs detected via program. | Windows Firewall Settings Have Been Changed | `rules/windows/builtin/firewall_as/win_firewall_as_setting_change.yml` | 0.35 | firewall |
...
| `5015084` | [WINDOWS-POWERSHELL] Windows Defender Uninstalled via P | Suspicious Uninstall of Windows Defender Feature via Po | `rules/windows/process_creation/proc_creation_win_powershell_uninstall_defender_feature.yml` | 0.35 | uninstall-windowsfeature, windows-defender, defender, powershell |
...
| `5007286` | "[SOPHOS] Website blocked | Tamper With Sophos AV Registry Keys | `rules/windows/registry/registry_set/registry_set_sophos_av_tamper.yml` | 0.35 | endpoint, sophos |
